...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, num_rnn_layers, cell_type='GRU'):
        ...
        # loop through RNN layers: just 2 though
        for i in range(num_rnn_layers):
            # get the class of a specific RNN cell type (nn.GRU, nn.LSTM) from the torch.nn
            rnn_cell_class = getattr(nn, cell_type)  # get RNN cell class based on the cell_type variable

            # Create an instance of the RNN cells with the appropriate input size and hidden state size
            # cell input size is determined by input_size_list[i]: 0=input_size; 1=hidden_state_size of layer bellow
            # the hidden state size is the same for all layers, defined by hidden_state_size.
            rnn_cell_instance = rnn_cell_class(input_size_list[i], hidden_state_size)
            rnn_cells.append(rnn_cell_instance)

        # wrap list of RNN cells in a nn.ModuleList and assign it to self.cells
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        print("current hidden state shape:", current_hidden_state.shape)
        ...
        print("input_tokens shape timepoint 0:", input_tokens.shape)
        ...
        for i in range(0, sequence_length):
            ...
            for j, cell in enumerate(self.cells):
                if j == 0:
                    print("shape processed_cnn_features", processed_cnn_features.shape)
                ...
                # current_hidden_state[j] = hidden_state.squeeze(0)
            ...
            logits_i = output_layer(rnn_output)
            ...
            predictions = torch.argmax(logits_i, dim=-1).squeeze(0)
            ...
            if i < sequence_length - 1:
                if is_train:
                    ...
                    print("input_tokens shape time-point TRAIN/CORRECT:", input_tokens.shape)
                else:
                    ...
                    # embedding_layer expects tensor of shape [batch_size, sequence_length]
                    input_tokens = embedding_layer(predictions).squeeze(1)
                    print("input_tokens shape time-point VALIDATE_1:", input_tokens.shape)
                    input_tokens = embedding_layer(predictions.unsqueeze(1))
                    print("input_tokens shape time-point VALIDATE_2:", input_tokens.shape)
                    input_tokens = embedding_layer(predictions)
                    print("input_tokens shape time-point VALIDATE_2:", input_tokens.shape)
                    # embedding_layer is an instance of nn.Embedding that maps word indices to their corresponding word embeddings
                    # input_tokens: each row represents the word embedding of the predicted word for the corresponding input sample in the batch.


        logits = torch.stack(logits_sequence, dim=1)  # Convert the sequence of logits to a tensor

        print("shape logits_i", logits_i.shape)
        print("shape logits", logits.shape)
        # shape  logits_i: torch.Size([1, 128, 10000])
        # shape  logits: torch.Size([128, 25, 10000])
        # shape logits_i torch.Size([1, 8, 10000])
        # shape logits torch.Size([8, 25, 10000])
